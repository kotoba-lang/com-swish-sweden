(ns swish-sweden.main
  "Kotodama WASM entrypoint for the Swish_sweden clean-room actor (L4) — Clojure port.
  ...
  facts (`swish_sweden.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "swish_sweden")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"swish_sweden.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "swish_sweden-compat" :tier tier :entities entities} 200])
...
(defn start! [] :swish_sweden-compat/ready)
